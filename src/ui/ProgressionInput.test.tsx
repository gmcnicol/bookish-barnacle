...
test('changing progression recomputes voicings and rerenders', () => {
  ...
  assert.equal(fretboard.getAttribute('data-frets'), '-1,3,2,0,-1,-1')
  assert.equal(tablature.getAttribute('data-frets'), '-1,3,2,0,-1,-1')
  fireEvent.change(input, { target: { value: 'V' } })
  fretboard = screen.getByTestId('fretboard')
  tablature = screen.getByTestId('tablature')
  assert.equal(fretboard.getAttribute('data-frets'), '-1,2,0,0,-1,-1')
  assert.equal(tablature.getAttribute('data-frets'), '-1,2,0,0,-1,-1')
})
